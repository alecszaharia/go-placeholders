...
func getValue(block *visitor.Block) {
	rand.Seed(time.Now().UnixNano())
	randomDuration := time.Duration(rand.Intn(200)+1) * time.Millisecond

	fmt.Printf("Get Value in [%s] .. ", randomDuration.String())
...
func main() {
	...
	//input := antlr.NewInputStream("aaa{{A  ty='t' }}wegweg{{end_A}}")
	...
	fmt.Printf("\n\n----------------------------------------------------------\n")
	fmt.Printf("Getting PLaceholder values \n")

	start2 := time.Now()
	for _, placeholder := range v.Blocks {
	...
	fmt.Printf("\n\n----------------------------------------------------------\n")
	fmt.Printf("Getting PLaceholder values concurently \n")
