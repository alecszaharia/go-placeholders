...
func getValue(block *visitor.Placeholder) {
	rand.Seed(time.Now().UnixNano())
	randomDuration := time.Duration(rand.Intn(50)+1) * time.Millisecond

	time.Sleep(randomDuration)

}

func prettyPrint(blocks []*visitor.Placeholder, indent string) {
	for _, b := range blocks {
		fmt.Println(indent + "-> " + b.Name)
		if len(b.Children) > 0 {
			prettyPrint(b.Children, indent+"  ")
		}
	}
}
